import{T as Ae,M as H,A as pe,a as f,P as k,G as N,V as C,S as L,R as G,b as oe,L as et,C as We,c as T,O as tt,B as He,F as Re,U as X,W as Y,H as q,N as Qe,d as st,e as O,f as Ue,g as ot,h as at,i as rt,j as it,k as nt,l as lt,m as ct,n as ht,o as je,p as ut,q as ft,r as dt,s as ie,t as pt,u as mt,v as gt,w as vt,x as xt,D as Fe,y as bt,z as Ee,E as ae,I as E,J as wt,K as Mt,Q as Ye,X as qe,Y as Xe,Z as $e,_ as S,$ as P,a0 as ee,a1 as ke,a2 as Be,a3 as yt}from"./three.module-CGvGw3gl.js";import{m as Ct}from"./BufferGeometryUtils-BSnYmD47.js";const Tt={vertexShader:`
    varying vec2 vUv;
    void main() {
      vUv = uv;
      ...
      gl_FragColor = vec4(color, alpha);
		...
		void main() {
			...
			gl_FragColor = mix( outputColor, texel, alpha );
				...
				}`})}}Q.BlurDirectionX=new C(1,0);Q.BlurDirectionY=new C(0,1);const te={defines:{DEPTH_PACKING:1,PERSPECTIVE_CAMERA:1},uniforms:{tColor:{value:null},tDepth:{value:null},focus:{value:1},aspect:{value:1},aperture:{value:.025},maxblur:{value:.01},nearClip:{value:1},farClip:{value:1e3}},vertexShader:`
  ...
  `};class Ht{constructor(e,t,s,o){this.renderer=e,this.scene=t,this.camera=s,this.composer=new Rt(e),this.renderPass=new Ut(t,s),this.composer.addPass(this.renderPass),o.quality!=="low"?(this.bloom=new Q(new C(1,1),.22,.7,.9),this.composer.addPass(this.bloom)):this.bloom=null,o.quality==="high"?(this.bokeh=new Et(t,s,{focus:14,aperture:.0013,maxblur:.008}),this.composer.addPass(this.bokeh)):this.bokeh=null,this.painterly=new Ze(Wt),this.painterly.uniforms.tAngle.value=Vt(),this.painterly.uniforms.uLength.value=o.quality==="high"?13:9,this.painterly.uniforms.uStrength.value=o.quality==="high"?.6:.52,this.composer.addPass(this.painterly),this.composer.addPass(new kt)}composer;painterly;bloom;bokeh;renderPass;width=1;height=1;setPainterly(e){this.painterly.uniforms.uStrength.value=e}setSize(e,t,s){this.width=e,this.height=t,this.composer.setPixelRatio(s),this.composer.setSize(e,t),this.painterly.uniforms.uTexel.value.set(1/(e*s),1/(t*s)),this.bloom?.setSize(e,t)}render(){this.composer.render()}get size(){return{width:this.width,height:this.height}}dispose(){this.composer.dispose(),this.bloom?.dispose(),this.bokeh?.dispose(),this.painterly.dispose(),this.renderPass.dispose()}}const _={sky:12575970,fog:3364442,bark:16771280,barkFar:10470604,foliage:15660258,foliageFar:14478570,grass:6581056,lionBody:15907683,lionBodyDark:14129727,lionMane:13664040,lionManeLight:16174207,muzzle:14264159,face:2890258,cloth:16777215,clothDark:9377296},Ke=42,fe=1.45,de=1;function Qt(i,e){const t=Math.hypot(e+7.4,fe-de),s=Math.tan(Ke/2*(Math.PI/180))*t;return{width:s*i,height:s}}function F(i){const e=i.map(s=>{const o=s.geometry.clone();return o.applyMatrix4(new Ye().compose(new T(...s.position??[0,0,0]),new qe().setFromEuler(new Xe(...s.rotation??[0,0,0])),new T(...s.scale??[1,1,1]))),o.index?o.toNonIndexed():o}),t=Ct(e,!1)??new He;for(const s of e)s.dispose();for(const s of i)s.geometry.dispose();return t}function j(i){return new E({color:i.color,map:i.map,roughness:i.roughness??.92,metalness:0})}function he(i,e,t){const s=[];for(let r=0;r<=14;r+=1){const l=r/14,n=1+Math.pow(1-l,3)*1.35,u=1-l*.42,h=1+Math.sin(l*Math.PI*3)*.035;s.push(new C(e*u*n*h,l*i))}const a=new $e(s,32);return a.computeVertexNormals(),new f(a,t)}function jt(i,e,t){const s=[];let o=t;const a=()=>(o=o*16807%2147483647,o/2147483647);for(let r=0;r<6;r+=1){const l=a()*Math.PI*2,n=a()*i*.55,u=i*(.42+a()*.3);s.push({geometry:new P(u,12,9),position:[Math.cos(l)*n,(a()-.35)*i*.45,Math.sin(l)*n],scale:[1.05,.85+a()*.25,1.05]})}return new f(F(s),e)}function Yt(){const e=new N,t=new E({color:_.lionBody,roughness:.9,metalness:0}),s=new E({color:_.lionBodyDark,roughness:.92,metalness:0}),o=new E({color:_.lionMane,roughness:.98,metalness:0,bumpMap:Ge(),bumpScale:.06}),a=new E({color:_.lionManeLight,roughness:.98,metalness:0,bumpMap:Ge(),bumpScale:.05});new E({color:_.muzzle,roughness:.85,metalness:0});const r=new E({color:_.face,roughness:.45,metalness:0});e.add(new f(F([{geometry:new S(.42,0),position:[-.36,.42,-.34],scale:[1,.95,1.15]},{geometry:new S(.42,0),position:[.36,.42,-.34],scale:[1,.95,1.15]},{geometry:new P(.44,26,20),position:[0,1.04,-.12],scale:[1.05,1.35,.95]},{geometry:new P(.44,26,20),position:[0,.9,-.58],scale:[1,.92,1.05]},{geometry:new P(.28,14,11),position:[0,1.46,.04],scale:[1.05,.9,1]}]),t));const l=[],n=[];for(const d of[-.24,.24]){l.push({geometry:new ee(.095,.115,.98,12),position:[d,.52,.22]}),n.push({geometry:new P(.14,12,10),position:[d,.09,.3],scale:[1.05,.62,1.5]});for(const v of[-.06,0,.06])n.push({geometry:new P(.045,8,6),position:[d+v,.05,.46]})}for(const d of[-.44,.44])l.push({geometry:new ee(.115,.135,.52,12),position:[d,.3,-.18],rotation:[1,0,0]}),n.push({geometry:new P(.15,12,10),position:[d,.09,.06],scale:[1.1,.62,1.6]});e.add(new f(F(l),t)),e.add(new f(F(n),s));const u=new f(new P(.44,26,20),t);u.position.set(0,1,.16),u.scale.set(1,1.15,.95),e.add(u);const h=new N;h.position.set(0,1.78,.14),h.add(new f(F([{geometry:new P(.29,26,20),scale:[1,.96,1.06]},{geometry:new ee(.09,.19,.36,16),position:[0,-.13,.3],rotation:[Math.PI/2,0,0],scale:[.86,1,1]},{geometry:new P(.09,10,8),position:[0,-.25,.3],scale:[1.2,.9,1]},{geometry:new ke(.34,.05,.14),position:[0,.12,.24]}]),t)),h.add(new f(new Be(.075,.1,3),r).translateY(0).translateZ(0));const c=h.children[h.children.length-1];c.position.set(0,-.08,.47),c.rotation.set(Math.PI/2,0,Math.PI),h.add(new f(new ke(.13,.012,.08),r).translateY(0).translateZ(0)),h.children[h.children.length-1].position.set(0,-.22,.4),h.add(new f(F([{geometry:new P(.062,10,8),position:[-.15,.05,.27],scale:[1.9,.3,.5],rotation:[0,0,.34]},{geometry:new P(.062,10,8),position:[.15,.05,.27],scale:[1.9,.3,.5],rotation:[0,0,-.34]}]),r));const x=[{geometry:new S(.66,1),position:[0,-.06,-.18],scale:[1.18,1.22,.6]},{geometry:new S(.3,0),position:[-.42,.06,.06],scale:[.95,1.15,.6],rotation:[0,0,.5]},{geometry:new S(.3,0),position:[.42,.06,.06],scale:[.95,1.15,.6],rotation:[0,0,-.5]},{geometry:new S(.26,0),position:[-.3,-.36,.16],scale:[.95,1.2,.6],rotation:[0,0,.25]},{geometry:new S(.26,0),position:[.3,-.36,.16],scale:[.95,1.2,.6],rotation:[0,0,-.25]},{geometry:new S(.3,0),position:[-.3,-.62,.02],scale:[1,1.3,.68]},{geometry:new S(.3,0),position:[.3,-.62,.02],scale:[1,1.3,.68]},{geometry:new S(.32,0),position:[0,-.8,.06],scale:[1.05,1.2,.68]},{geometry:new S(.42,0),position:[-.56,-.24,-.1],scale:[.95,1.15,.62]},{geometry:new S(.42,0),position:[.56,-.24,-.1],scale:[.95,1.15,.62]}],w=[{geometry:new S(.34,0),position:[0,.34,-.04],scale:[1.2,.8,.7]},{geometry:new S(.28,0),position:[-.5,.16,-.02],scale:[.95,1,.62],rotation:[0,0,.35]},{geometry:new S(.28,0),position:[.5,.16,-.02],scale:[.95,1,.62],rotation:[0,0,-.35]}];h.add(new f(F(x),o)),h.add(new f(F(w),a));const g=[];for(const d of[-.23,.23]){const v=new f(new Be(.095,.17,5),a);v.position.set(d,.3,.06),v.rotation.z=d<0?.4:-.4,h.add(v),g.push(v)}e.add(h);const M=[];let A=e;for(let d=0;d<2;d+=1){const v=new N;v.position.set(0,d===0?.5:.02,d===0?-.94:-.3);const D=new f(new ee(.07-d*.012,.08-d*.012,.32,10),t);if(D.rotation.x=Math.PI/2+.5,D.position.z=-.16,v.add(D),d===1){const U=new f(new S(.11,0),o);U.scale.set(.9,1.5,.9),U.position.set(0,-.18,-.3),v.add(U)}A.add(v),A=v,M.push(v)}return e.scale.setScalar(1.16),{group:e,head:h,headBaseY:1.34,tail:M,ears:g,chest:u}}function qt(){const i=new N,e=j({color:_.cloth,map:zt(),roughness:.95}),t=new E({color:_.clothDark,roughness:.9,metalness:0}),s=[],o=16;for(let h=0;h<=o;h+=1){const c=h/o,p=.16+Math.pow(c,1.5)*.3+Math.sin(c*Math.PI)*.05;s.push(new C(p,1.02-c*1))}const a=new $e(s,32);a.computeVertexNormals();const r=new f(a,e);i.add(r);const l=new f(new P(.18,16,12),e);l.scale.set(1,1.12,1.08),l.position.set(0,1.06,.01),i.add(l);const n=new f(new P(.2,14,11),e);n.scale.set(1.5,.6,.9),n.position.set(0,.92,0),i.add(n);const u=new f(new P(.1,12,10),t);return u.scale.set(1,1.15,.6),u.position.set(0,1.05,.13),i.add(u),i.scale.setScalar(.68),{group:i,cloak:r,basePositions:Float32Array.from(a.attributes.position.array)}}function Xt(i){const e=new k(.17,.24),t=new E({map:Nt(),transparent:!0,alphaTest:.4,side:ae,roughness:1,metalness:0}),s=new yt(e,t,i);s.frustumCulled=!1;const o=new Float32Array(i*6);for(let a=0;a<i;a+=1)o[a*6+0]=-6+Math.random()*12,o[a*6+1]=.4+Math.random()*4.8,o[a*6+2]=-5+Math.random()*10,o[a*6+3]=.5+Math.random()*1.2,o[a*6+4]=Math.random()*Math.PI*2,o[a*6+5]=(Math.random()-.5)*3;return{mesh:s,state:o}}function Ve(){try{const s=new URLSearchParams(window.location.search).get("fx");if(s==="low"||s==="medium"||s==="high")return s}catch{}const i=typeof window.matchMedia=="function"&&window.matchMedia("(pointer: coarse)").matches,e=navigator.hardwareConcurrency??4;return!i&&e>=4?"high":e>=4?"medium":"low"}class Kt{constructor(e){this.canvas=e,this.renderer=new pt({canvas:e,antialias:!0,powerPreference:"low-power"}),this.renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,2)),this.renderer.shadowMap.enabled=!0,this.renderer.shadowMap.type=mt,this.renderer.toneMapping=je,this.renderer.toneMappingExposure=1,this.scene.background=new O(0),this.scene.fog=new gt(_.fog,16,30),this.camera=new vt(Ke,2/3,.1,90),this.camera.position.set(0,fe,7.4),this.camera.lookAt(0,de,0);const t=Lt();this.scene.add(new xt(14479344,3498094,1.05));const s=new Fe(16761976,1.15);s.position.set(4,7.5,-6),s.castShadow=!0,s.shadow.mapSize.set(512,512),s.shadow.camera.left=-7,s.shadow.camera.right=7,s.shadow.camera.top=8,s.shadow.camera.bottom=-2,s.shadow.bias=-.0012,s.shadow.radius=3,this.scene.add(s);const o=new Fe(8832742,1.5);o.position.set(-5,3,5),this.scene.add(o);const a=new bt(16767392,13,12,Math.PI/9,.8,1.6);a.position.set(-1.4,4.2,4.6),a.target.position.set(-.3,1,1.6),this.scene.add(a),this.scene.add(a.target);const r=new k(70,40,1,8),l=r.attributes.position,n=new Float32Array(l.count*3),u=new O(_.sky),h=new O(_.fog);for(let m=0;m<l.count;m+=1){const R=(l.getY(m)+20)/40,b=h.clone().lerp(u,R);n[m*3]=b.r,n[m*3+1]=b.g,n[m*3+2]=b.b}r.setAttribute("color",new Ee(n,3));const c=new H({vertexColors:!0,fog:!1,side:ae}),p=new f(r,c);p.position.set(0,8,-22),this.addForest(p),this.disposables.push(r,c);const x=new k(46,46,18,18),w=x.attributes.position,g=new Float32Array(w.count*3),M=new O;for(let m=0;m<w.count;m+=1){const R=w.getX(m),b=w.getY(m);w.setZ(m,Math.sin(R*.28)*.09+Math.cos(b*.24)*.07);const I=(Math.sin(R*.5)+Math.cos(b*.42))*.5,W=I>.35?1.16:I<-.35?.82:1,J=Math.min(1,Math.max(0,b/23));M.setHex(_.grass).multiplyScalar(W*(1-J*.3)),g[m*3]=M.r,g[m*3+1]=M.g,g[m*3+2]=M.b}x.setAttribute("color",new Ee(g,3)),x.computeVertexNormals();const A=new E({map:Bt(),vertexColors:!0,roughness:1,metalness:0,bumpMap:t,bumpScale:.02}),d=new f(x,A);d.rotation.x=-Math.PI/2,d.receiveShadow=!0,this.ground=d,this.scene.add(d),this.disposables.push(x,A);const v=j({color:_.bark,map:Le()}),D=j({color:_.barkFar,map:Le(),roughness:1}),U=j({color:_.foliage,map:Ie(),roughness:1}),ce=j({color:_.foliageFar,map:Ie(),roughness:1});this.disposables.push(v,D,U,ce);const $=new N;$.name="leftFrame";const ge=he(13,.6,v);ge.castShadow=!0,$.add(ge),$.add(this.placeCanopy(11.6,1.05,U,7)),this.addForest($);const Z=new N;Z.name="rightFrame";const ve=he(13.6,.66,D);ve.castShadow=!0,Z.add(ve),Z.add(this.placeCanopy(12.2,1.1,U,11)),this.addForest(Z);const Je=[[-1.6,-10,12],[.9,-11,13],[-3,-15,15],[2.8,-16,15]],xe=[],be=[];Je.forEach(([m,R,b],I)=>{const W=he(b,b*.05,D).geometry.clone();W.translate(m,0,R),xe.push({geometry:W});const J=this.placeCanopy(b*.92,b*.16,ce,I*13+5),De=J.geometry.clone();De.translate(m,b*.92,R),be.push({geometry:De}),J.geometry.dispose()});const we=new f(F(xe),D);we.name="farTrunks";const Me=new f(F(be),ce);Me.name="farCanopies",this.addForest(we),this.addForest(Me);const K=new N;K.name="canopyTop",K.add(this.placeCanopy(0,.55,U,3)),this.addForest(K),this.foliage.push(K);for(const[m,R]of[["canopyleft",-1],["canopyright",1]]){const b=new N;b.name=m,b.add(this.placeCanopy(0,.7,U,R<0?17:23)),this.addForest(b),this.foliage.push(b)}const ye=new H({map:It(),transparent:!0,blending:pe,depthWrite:!1,side:ae,opacity:.85}),Ce=[];for(const[m,R,b,I,W]of[[-2.2,-3.2,1.5,9,.14],[-.6,-4.4,1.9,10,.1],[1.4,-3.6,1.4,8.5,-.12]])Ce.push({geometry:new k(b,I),position:[m,I/2-.4,R],rotation:[0,0,W]});const Te=new f(F(Ce),ye);Te.renderOrder=2,this.addForest(Te),this.disposables.push(ye);const Se=new H({map:Gt(),transparent:!0,depthWrite:!1,side:ae,opacity:.55}),_e=[];for(const[m,R,b]of[[-6,.7,1.6],[-12,1.1,2.2]])_e.push({geometry:new k(26*b,3.4*b),position:[0,R,m]});const Pe=new f(F(_e),Se);Pe.renderOrder=1,this.addForest(Pe),this.disposables.push(Se),this.lion=Yt(),this.lion.group.position.set(-.3,0,1.6),this.lion.group.rotation.y=-.12,this.lion.group.scale.setScalar(.85),this.lion.group.traverse(m=>{m instanceof f&&(m.castShadow=!0)}),this.scene.add(this.lion.group),this.traveler=qt(),this.traveler.group.position.set(.35,0,2.6),this.traveler.group.rotation.y=Math.PI-.25,this.traveler.group.scale.setScalar(.6),this.traveler.group.traverse(m=>{m instanceof f&&(m.castShadow=!0)}),this.scene.add(this.traveler.group),this.leaves=Xt(45),this.scene.add(this.leaves.mesh),this.disposables.push(this.leaves.mesh.geometry,this.leaves.mesh.material),this.postfx=new Ht(this.renderer,this.scene,this.camera,{quality:Ve()}),this.resize(e.clientWidth,e.clientHeight),this.layout(),this.hideProcedural(),this.canvas.style.opacity="0",this.loadPlates(),window.setTimeout(()=>{this.loaded.plates||(this.canvas.style.opacity="1",this.showProcedural())},6e3),this.animate()}renderer;postfx;scene=new wt;camera;clock=new Mt;lion;forest=[];loaded={plates:!1};plates=null;spray=null;ground=null;traveler;leaves;foliage=[];matrix=new Ye;disposables=[];frame=0;fpsAccum=0;fpsFrames=0;fps=0;wind=0;get assets(){return{...this.loaded}}get quality(){return this.postfx?this.postfx.constructor.name:"нет"}placeCanopy(e,t,s,o){const a=jt(t,s,o);return a.position.y=e,a}alignToNdc(e,t,s,o){e.position.z=o;const a=new T;for(let r=0;r<8;r+=1){e.getWorldPosition(a),a.project(this.camera);const{width:l,height:n}=Qt(this.camera.aspect,o);e.position.x+=(t-a.x)*l*.85,e.position.y+=(s-a.y)*n*.85}}layout(){const t=this.scene.getObjectByName("leftFrame"),s=this.scene.getObjectByName("rightFrame");t&&this.alignToNdc(t,-.86,-.5,-1.2),s&&this.alignToNdc(s,.86,-.5,-1.2);const o=this.scene.getObjectByName("canopyTop");o&&this.alignToNdc(o,0,1.06,-1.2-.6);for(const[a,r]of[["canopyleft",-1],["canopyright",1]]){const l=this.scene.getObjectByName(a);l&&this.alignToNdc(l,r*.9,.92,-1.2)}}addForest(e){this.scene.add(e),this.forest.push(e)}loadPlates(){const e=new me(this.camera,{base:"./title/layers/",quality:Ve(),animate:!1,onReady:()=>{this.loaded.plates=!0,this.hideProcedural(),this.canvas.style.opacity="1",this.postfx?.setPainterly(.18)}});this.plates=e,this.scene.add(e.object3d)}showProcedural(){for(const e of this.forest)e.visible=!0;this.ground&&(this.ground.visible=!0),this.lion.group.visible=!0,this.traveler.group.visible=!0}hideProcedural(){for(const e of this.forest)e.visible=!1;this.ground&&(this.ground.visible=!1),this.lion.group.visible=!1,this.traveler.group.visible=!1}resize(e,t){const s=e/Math.max(1,t);this.camera.aspect=s;const o=7.4+Math.max(0,s-1)*3;this.camera.position.set(0,fe+Math.max(0,s-1)*.4,o),this.camera.lookAt(0,de,0),this.camera.updateProjectionMatrix(),this.renderer.setSize(e,t,!1),this.postfx.setSize(e,t,Math.min(window.devicePixelRatio||1,2)),this.layout(),this.plates?.layout()}setPointer(e,t){this.plates?.setPointer(e,t)}animate=()=>{this.frame=requestAnimationFrame(this.animate);const e=Math.min(this.clock.getDelta(),.05);if(document.hidden)return;const t=this.clock.elapsedTime;this.wind=Math.sin(t*.55)*.6+Math.sin(t*1.7)*.25+.4,this.plates?.update(t),this.spray?.update(e,this.wind);const s=1+Math.sin(t*2.1)*.025;this.lion.chest.scale.setScalar(s),this.lion.head.rotation.y=Math.sin(t*.45)*.12,this.lion.head.rotation.x=Math.sin(t*.7+1)*.05,this.lion.head.position.y=this.lion.headBaseY+Math.sin(t*2.1)*.014,this.lion.ears.forEach((h,c)=>{const p=Math.max(0,Math.sin(t*.9+c*2.1)-.93)*12;h.rotation.z=(c===0?.34:-.34)+p*(c===0?1:-1)}),this.lion.tail.forEach((h,c)=>{h.rotation.y=Math.sin(t*1.1-c*.5)*(.12+c*.05)*(.8+this.wind*.6),h.rotation.x=-.1+Math.sin(t*.8-c*.4)*.08});const o=this.traveler.cloak,a=o.geometry.attributes.position,r=this.traveler.basePositions;for(let h=0;h<a.count;h+=1){const c=r[h*3]??0,p=r[h*3+1]??0,x=r[h*3+2]??0,w=Math.max(0,.55-p)/.55,g=Math.atan2(x,c),M=this.wind*(.14+.06*Math.sin(t*2.6+g*3));a.setXYZ(h,c+Math.sin(g)*w*M+Math.sin(t*3+g*2)*w*.014,p-Math.abs(M)*w*.07,x+Math.cos(g)*w*M+Math.cos(t*2.4+g*2)*w*.014)}a.needsUpdate=!0,o.geometry.computeVertexNormals(),this.foliage.forEach((h,c)=>{h.rotation.z=Math.sin(t*.9+c)*.028*(.6+this.wind)});const{mesh:l,state:n}=this.leaves,u=n.length/6;for(let h=0;h<u;h+=1){const c=h*6;let p=n[c]??0,x=n[c+1]??0,w=n[c+2]??0;const g=n[c+3]??1,M=n[c+4]??0,A=n[c+5]??1;p+=(g*(.6+this.wind)+.35)*e*1.7,x+=Math.sin(t*1.6+M)*e*.5,w+=Math.cos(t*1.1+M)*e*.3,p>8&&(p=-8,x=.4+Math.random()*4.8,w=-5+Math.random()*10),n[c]=p,n[c+1]=x,n[c+2]=w;const d=.85+Math.sin(t*2+M)*.15;this.matrix.compose(new T(p,x,w),new qe().setFromEuler(new Xe(t*A,M+t*.8,Math.sin(t*1.5+M)*.7)),new T(d,d,d)),l.setMatrixAt(h,this.matrix)}l.instanceMatrix.needsUpdate=!0,this.postfx.render(),this.fpsAccum+=e,this.fpsFrames+=1,this.fpsAccum>=.5&&(this.fps=Math.round(this.fpsFrames/this.fpsAccum),this.fpsAccum=0,this.fpsFrames=0)};get stats(){let e=0,t=0;return this.scene.traverse(s=>{if(!(s instanceof f))return;e+=1;const o=s.geometry,a=o.getAttribute("position"),r=o.getIndex();r?t+=r.count/3:a&&(t+=a.count/3)}),{calls:e,triangles:Math.round(t),fps:this.fps}}get objectCount(){let e=0;return this.scene.traverse(()=>{e+=1}),e}screenPositionOf(e){const t=this.scene.getObjectByName(e);if(!t)return null;const s=new T;return t.getWorldPosition(s),s.project(this.camera),{x:s.x,y:s.y}}dispose(){this.spray?.dispose(),this.spray=null,cancelAnimationFrame(this.frame),this.postfx.dispose();for(const e of this.disposables)e.dispose();this.scene.traverse(e=>{if(e instanceof f){e.geometry.dispose();for(const t of Array.isArray(e.material)?e.material:[e.material])t.dispose()}}),this.scene.clear(),Ot(),this.renderer.dispose()}}export{_ as TITLE_PALETTE,Kt as TitleScene};
//# sourceMappingURL=titleScene-xd_-ptd1.js.map
